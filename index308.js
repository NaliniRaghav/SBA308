...
function getLearnerData(CourseInfo, AssignmentGroup, LearnerSubmissions) {
  ...
  LearnerSubmissions.forEach(function(submission) {
      var assignment = AssignmentGroup.assignments.find(function(a) {
      ...
      var score = submission.submission.score;

      // Extract the date part (YYYY-MM-DD) from due date and submitted date
      var dueDate = new Date(assignment.due_at).toISOString().split('T')[0];
      var submittedDate = new Date(submission.submission.submitted_at).toISOString().split('T')[0];
      ...
      var percentage = (score / assignment.points_possible) * 100;

      var learner = learnerData.find(function(l) {
...
function displayResults(results, title) {
  ...
  results.forEach(function(learner) {
      ...
      var learnerId = document.createElement('h3');
     ...
     avg.style.color= 'blue';
      ...
      var assignmentsList = document.createElement('ul');

      // Retrieve all keys from the learner object
      var keys = Object.keys(learner);
...
// Valid Data
try {
  var results = getLearnerData(CourseInfo, AssignmentGroup, LearnerSubmissions);
...
try {
  var invalidResults = getLearnerData(CourseInfo, AssignmentGroup, invalidLearnerSubmissions);

...
function getLearnerData(CourseInfo, AssignmentGroup, LearnerSubmissions) {
  ...
  LearnerSubmissions.forEach(function (submission) {
    ...
    if (!assignment) {
      throw new Error(
        "Assignment with id " + submission.assignment_id + " not found."
      );
    }
      ...
      switch (key) {
        ...
        default:
          ...
          listItem.textContent =
            "Assignment " + key + ": " + learner[key].toFixed(2) + "%";
...
try {
  ...
  document.getElementById("output").innerHTML +=
    "<p>Error with valid data: " + error.message + "</p>";
}
...
try {
  let invalidResults = getLearnerData(
    CourseInfo,
    AssignmentGroup,
    invalidLearnerSubmissions
  );
  displayResults(invalidResults, "Invalid Data");
} catch (error) {
  document.getElementById("output").innerHTML +=
    '<p class="average">Error with invalid data: ' + error.message + "</p>";
}
